...
export default function Services() {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            h1 { color: #171130; text-align: center; margin-bottom: 5px; }
            h2 { color: #666; text-align: center; margin-top: 0; margin-bottom: 30px; font-weight: normal; font-size: 18px; }
            h3 { color: #c8723e; border-bottom: 2px solid #c8723e; padding-bottom: 5px; margin-top: 25px; }
  ...
  return (
    <section id="services" className="py-20 bg-brand-bg border-b-2 border-brand-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service) => (
            <div
              key={service.title}
              className={`group ${service.bgColor} border border-brand-border p-6 rounded-2xl shadow-sm hover:shadow-md transition-all flex flex-col`}
            >
              {/* Icon */}
              <div className="w-14 h-14 bg-brand-bg border border-brand-border rounded-xl flex items-center justify-center text-2xl mb-6 shadow-sm">
                {service.icon}
              </div>

              <h3 className="text-brand-text font-bold text-xl mb-2 uppercase tracking-wider">
                {service.title}
              </h3>
              <p className="text-brand-text-light font-medium text-xs mb-6 flex-1">
                {service.description}
              </p>
              ...
              <ul className="space-y-3 mb-6">
                {service.benefits.map((benefit) => (
                  <li key={benefit} className="flex items-center gap-3 text-[10px] font-bold text-brand-text uppercase tracking-wider">
              ...
              <div className="flex gap-3 mt-auto">
                <button
                  ...
                  className="flex-1 flex items-center justify-center bg-brand-bg border border-brand-border text-brand-text text-xs font-bold py-3 uppercase tracking-wider rounded-xl shadow-sm hover:bg-brand-surface transition-colors"
                >
                  View Details
                </button>
                <a
      ...
      {selectedService && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-brand-surface border border-brand-border rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col animate-fade-in-up">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-6 border-b border-brand-border bg-brand-navy rounded-t-2xl">
              <div>
                <h3 className="text-xl font-bold text-brand-text uppercase tracking-wider">
              ...
              <button
                onClick={() => setSelectedService(null)}
                className="w-8 h-8 flex items-center justify-center bg-brand-bg border border-brand-border rounded-lg text-brand-text hover:bg-brand-surface transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-8 custom-scrollbar">
              ...
              {/* KYC - Always show */}
              <div>
                <div className="flex items-center gap-3 mb-4 border-b border-brand-border pb-2">
                  <span className="text-2xl">👤</span>
                  <h4 className="text-sm font-bold text-brand-text uppercase tracking-wider">KYC Documents (For All)</h4>
                </div>
                <ul className="space-y-3">
                  {KYC_DOCUMENTS.map((doc, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <span className="text-[#4ade80] mt-0.5">✓</span>
                      <span className="text-sm font-medium text-brand-text-light">{doc}</span>
                    </li>
                  ))}
                </ul>
              </div>
                ...
                CHECKLISTS[selectedService].map((section, sIdx) => (
                  <div key={sIdx}>
                    <div className="flex items-center gap-3 mb-4 border-b border-brand-border pb-2">
                      ...
                      <h4 className="text-sm font-bold text-brand-text uppercase tracking-wider">{section.category}</h4>
                    </div>
                    <ul className="space-y-3">
              ...
              {CHECKLISTS[selectedService] && (
                <div>
                  <div className="flex items-center gap-3 mb-4 border-b border-brand-border pb-2">
                    <span className="text-xl">📌</span>
                    <h4 className="text-sm font-bold text-brand-text uppercase tracking-wider">Additional Documents (If Applicable)</h4>
                  </div>
                  <ul className="space-y-3">
                    {COMMON_ADDITIONAL.map((doc, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                        <span className="text-brand-text-light mt-0.5">✓</span>
                        <span className="text-sm font-medium text-brand-text-light">{doc}</span>
                      </li>
                    ))}
                  </ul>
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-brand-border bg-brand-navy rounded-b-2xl flex flex-col sm:flex-row gap-4 justify-between items-center">
              <button
                onClick={handleDownloadPDF}
                className="flex items-center gap-2 px-5 py-3 text-brand-text font-bold text-xs uppercase tracking-wider hover:text-brand-blue transition-colors"
              >
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <button
                  onClick={() => setSelectedService(null)}
                  className="flex-1 sm:flex-none px-6 py-3 bg-brand-bg border border-brand-border text-brand-text font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-brand-surface transition-colors"
                >
                  Close
                </button>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 sm:flex-none px-6 py-3 bg-[#25D366] text-white font-bold text-xs uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 hover:bg-[#20ba56] transition-colors"
                >
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #2e2260; border-radius: 4px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #c8723e; }
      `}} />
    </section>
  );
}
